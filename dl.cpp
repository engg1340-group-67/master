...
public:
	...
	Grad* gradient(vector<double> x, vector<double> target) {
		...
		this->final_layer = new Softmaxloss;*/
		layer();
		this->loss(x, target);//do forward and the help storing data
...
Grad* DL_main(vector<vector<double>> params_1, vector<double> const_1,
	...
	cin >> learning_rate;
	system("cls");
	cout << "Your learning rate : " << learning_rate << endl;
...
vector<double> DL(string name) {//forward
	double temp_num = 0;
	vector<double> temp_vec;
	string from;
	string to;
	string confirm;
	cout << "Training Datas from which day till Today" << endl;

	cout << "From(MMDD) - Today(MMDD), i.e 0701->1st July" << endl;
	cout << "From :"; cin >> from; cout << endl;
	cout << "Today :"; cin >> to; cout << endl;
	cout << "No skip days(Y/N)"; cin >> confirm;
	if (confirm == "N") {
		cout << "If you skip many day during the perioid/n please re-enter it by initialization in main menu" << endl;
		cout << "Or the data will be very inaccurate" << endl;
		cout << "Please use it everyday" << endl;
		exit(1);
	}
...
vector<double> deep_learning_main(string name) {
	char init = 'a';
	cout << "Deep Learning Initialize" << endl;
	cout << "If you haven't do it before, please do it or bug" << endl;
	cout << "(Y/N)";
	cin >> init;
	cout << endl;
	if (init == 'Y') {
	...
	vector<double> mypredict = DL(name);
	/*for (int i = 0; i < 6; i++) {
